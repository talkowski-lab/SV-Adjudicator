...
class SRTestRunner(PESRTestRunner):
    def __init__(self, vcf, countfile, fout, n_background=773, window=100,
    ...
    def null_score(self,record, null_val=0.0):
        ID=record.id
        d={'name':[ID,ID,ID],'pos':[null_val]*3,'coord': ['posA','posB','sum'],'log_pval':[null_val]*3,'called':[null_val]*3,'background':[null_val]*3}
